# log-024 マッチング離脱時に待機列へ自分が残る問題の修正
...
|---|---|
| ログ番号 | log-024 |
| 状態 | 🚧 実装・ローカル検証サーバーで確認済み／**実サーバーでの再現確認は未** |
| 担当AI | Claude Code (claude-opus-5) |
| 起票日 | 2026-08-01 |
| 完了日 | — |
| 関連ブランチ / PR | `feature/result-tweaks` |
| 関連ファイル | `src/App.tsx` / `src/state/reducer.ts` / `src/state/useGameState.ts` / `src/state/index.ts` |
