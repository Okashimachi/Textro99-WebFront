# docs/worklog — AI作業ログ（**運用終了・アーカイブ**）

> ⚠️ **この運用は log-024 で終了した。新しいログを起票しない。以下の記述は過去ログを読むための説明であり、現在のルールではない。**
> 調整フェーズ以降、作業の記録は PR 本文に集約する（[docs/rules/04-PRとレビュー.md](../rules/04-PRとレビュー.md)）。

AI（Claude Code / Codex / Cursor 等）に出した指示・AIの判断と決定・テスト仕様と結果・トークン消費を、**1タスク＝1ディレクトリ**で記録する場所。記録は関心事ごとにファイルへ切り分ける。

## 目的

- 「なぜこの実装になったか」「どの指示から生まれたか」を後から追える状態にする（AI生成コードの意思決定ログ）。
- テスト仕様と結果を残し、回帰時の確認点を明確にする。
- **トークン消費量とその原因/過程を記録し**、コスト効率の悪い進め方を可視化・改善する。

## 命名規則

```
docs/worklog/log-NNN-{短いkebab-caseの主題}/
```

- `NNN` は3桁連番（`001` から。**欠番を作らない**）。
- 例: `log-001-webfront-scaffold/` / `log-002-daken-display-component/`
- テンプレートは [_template/](./_template/)。新規タスク着手時にこのディレクトリごとコピーして番号・主題にリネームする。

## ディレクトリ内の構成（関心事ごとに切り分け）

| ファイル | 内容 | 記入タイミング |
|---|---|---|
| `README.md` | メタ情報（状態・担当AI・日付・ブランチ）と入口 | 着手時 |
| `01-指示.md` | 人間 → AI の指示 | 着手時 |
| `02-背景と前提.md` | 背景・参照した上流ドキュメント・制約 | 着手時 |
| `03-判断と決定.md` | AIの判断・決定・却下案・人間承認 | 作業中〜完了 |
| `04-成果物.md` | 変更ファイル・実装要点 | 完了時 |
| `05-テスト仕様.md` | 確認項目・手順・期待結果 | テスト時 |
| `06-テスト結果.md` | 合否・実挙動・対応 | テスト時 |
| `07-トークン消費.md` | 消費量・原因/過程・改善案 | 完了時（必須） |
| `08-振り返り.md` | 知見・上流への還元候補 | 完了時 |

## 索引

| # | タスク | 状態 | 日付 |
|---|---|---|---|
| [001](./log-001-proto-vendoring/README.md) | proto の TS 型取り込み（手コピー方式） | ✅ 完了（PR待ち） | 2026-07-23 |
| [002](./log-002-webfront-scaffold/README.md) | Vite+React+TS+Tailwind 基盤スキャフォールド（#3） | ✅ 完了（PR待ち） | 2026-07-23 |
| [003](./log-003-ws-dispatch/README.md) | WebSocket接続・メッセージディスパッチ層（#4） | ✅ 完了（PR待ち） | 2026-07-23 |
| [004](./log-004-state-reducer/README.md) | 受信stateのreducer/ViewModel（#5） | ✅ 完了（PR待ち） | 2026-07-23 |
| [005](./log-005-debug-pane/README.md) | RawStateDebugPane＋devモック（#6） | ✅ 完了（PR待ち） | 2026-07-23 |
| [006](./log-006-lifecycle/README.md) | 画面ライフサイクル状態機械（#9） | ✅ 完了（PR待ち） | 2026-07-23 |
| [007](./log-007-input/README.md) | 入力送信層（文字/Enter/0-9→各C2S）（#7） | ✅ 完了（PR待ち） | 2026-07-23 |
| [008](./log-008-hud/README.md) | 試合中HUDコンポーネント群（#11） | ✅ 完了（PR待ち） | 2026-07-23 |
| [009](./log-009-player-grid/README.md) | PlayerGrid99（99人ミニ盤面）（#12） | ✅ 完了（PR待ち） | 2026-07-23 |
| [010](./log-010-matchmaking/README.md) | マッチング待機画面（#10） | ✅ 完了（PR待ち） | 2026-07-23 |
| [011](./log-011-result/README.md) | リザルト画面（GameOver）（#13） | ✅ 完了（PR待ち） | 2026-07-23 |
| [012](./log-012-blocked-issues/README.md) | ブロック中 Issue の記録（#8 TypingJudge / #14 デプロイ） | ⏸ 保留 | 2026-07-23 |
| [014](./log-014-typing-judge/README.md) | タイピング判定(#8)＋フロント完結テスト/サーバー切替 | ✅ 実装・ブラウザ検証済み | 2026-07-26 |
| [015](./log-015-screen-flow/README.md) | 画面フロー(title→mode→name→in-game)＋名前・ランキング土台 | ✅ 実装・ブラウザ検証済み | 2026-07-26 |
| [016](./log-016-romaji/README.md) | ローマ字入力（かな→受理ローマ字）判定・#8 本実装 | ✅ 実装・ブラウザ検証済み | 2026-07-27 |
| [017](./log-017-hud-console-redesign/README.md) | 試合中HUDの「戦況コンソール」デザイン刷新（白地＋赤） | ✅ 実装・ブラウザ検証済み | 2026-07-29 |
| [018](./log-018-proto-v0.1.1/README.md) | proto v0.1.1 追従（AttackRequest 廃止・サーバー確定順位） | 🚧 実装済み・ブラウザ/実サーバー検証は未 | 2026-07-31 |
| [019](./log-019-play-flow/README.md) | 実プレイ用フロー（プレイする / テスト用の入口分離） | 🚧 実装済み・待機画面2状態は未検証 | 2026-07-31 |
| [020](./log-020-matchmaking-waiting-info/README.md) | マッチング待機画面に残り時間と参加者一覧を表示（proto 同期） | ✅ 実装・ブラウザ/実サーバー検証済み | 2026-08-01 |
| [021](./log-021-result-session/README.md) | リザルトのモーダル化とマッチングセッションの終端制御（自動で次試合に行くバグ修正） | 🚧 実装・ブラウザ（練習）検証済み・実サーバー検証は未 | 2026-08-01 |
| [022](./log-022-x-share-post/README.md) | リザルトの X 共有ボタン（Web Intent・投稿テキストはモック） | 🚧 実装・ブラウザ（練習）検証済み・実サーバー検証は未 | 2026-08-01 |
| [023](./log-023-result-tweaks/README.md) | リザルト・入力まわりのまとめ修正（文面切り出し/猶予30秒/バッジ非表示/数字・伸ばし棒/テストモード非表示） | 🚧 実装・ブラウザ（練習）検証済み・実サーバー検証は未 | 2026-08-01 |
| [024](./log-024-matchmaking-leave/README.md) | マッチング離脱時に待機列へ自分が残る問題の修正（離脱処理の一本化） | 🚧 実装・ローカル検証サーバーで確認済み・実サーバー再現確認は未 | 2026-08-01 |
