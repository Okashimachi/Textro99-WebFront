...
**作業ログ（`docs/worklog/`）の運用は終了した。新しく起票しない。**

調整フェーズに入ったため、作業の記録は **PR 本文**（[docs/rules/04-PRとレビュー.md](./docs/rules/04-PRとレビュー.md)）に集約する。判断の経緯や確認したことは PR に書けばよく、リポジトリ内に別途ドキュメントを作る必要はない。

> `docs/worklog/` に残っている過去ログは初期設計の経緯を追うための読み物であり、更新の対象ではない。
