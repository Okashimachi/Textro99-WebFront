...
const KIND_STYLE: Record<GameEvent["kind"], { mark: string; color: string }> = {
  Welcome: { mark: "•", color: "text-sub" },
  Ko: { mark: "✕", color: "text-accent" },
  AttackFailed: { mark: "⚔", color: "text-warn" },
  OffsetResolved: { mark: "▣", color: "text-ok" },
  GameOver: { mark: "■", color: "text-ink" },
};
...
export function EventLog({ events, limit = 6 }: Props) {
  ...
  return (
    <Panel label="戦況ログ" right={`${events.length} 件`} bodyClassName="p-2">
      {shown.length === 0 ? (
        <div className="py-1 text-[11px] text-sub">まだ動きはありません</div>
      ) : (
        <ul className="space-y-px">
          {shown.map((e) => {
            ...
            return (
              <li
                key={e.id}
                className="flex items-start gap-2 border-b border-line/60 px-1 py-1 text-xs leading-snug last:border-b-0"
              >
                <span className="w-8 shrink-0 text-[10px] tabular-nums text-sub">
                  {elapsed(e.atMs, baseMs)}
                </span>
                <span className={`w-3 shrink-0 text-center ${s.color}`} aria-hidden>
                  {s.mark}
                </span>
                <span className="min-w-0 flex-1 text-ink">{e.message}</span>
              </li>
            );
          })}
        </ul>
